...
(function() {
  ...
  function getCorePageSlug() {
    ...
    const match = path.match(/\/reference\/foundations\/([^/]+)\.html$/);
  ...
  function extractCoreStyleRules() {
    ...
    for (const sheet of document.styleSheets) {
      ...
      const isCoreSheet = /\/dist\/css\/core\.css$/i.test(href) || /\\dist\\css\\core\.css$/i.test(href) || /dist\/css\/core\.css/i.test(href);
      if (!isCoreSheet) {
        continue;
      }
      try {
  ...
  function renderClassReferenceList(mainEl, slug, classesMap) {
    ...
    panel.className = "fx-demo-panel fx-border fx-rounded fx-p-3";
